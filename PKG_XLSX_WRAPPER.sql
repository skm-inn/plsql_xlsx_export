-- Create XLSX_EXPORT_RESULTS table (idempotent)
BEGIN
  EXECUTE IMMEDIATE q'[
...
-- Create XLSX_EXPORT_LOG audit table (idempotent)
BEGIN
  EXECUTE IMMEDIATE q'[
...
CREATE OR REPLACE PACKAGE BODY PKG_XLSX_WRAPPER AS
  ...
  BEGIN
    LOOP
      ...
      dbg('unique_workbook_name: "' || UPPER(TRIM(p_base)) || '" exists → trying with suffix _' || LPAD(v_seq,2,'0'));
      v_name := UPPER(TRIM(p_base)) || '_' || LPAD(v_seq, 2, '0');
      v_seq  := v_seq + 1;
      EXIT WHEN v_seq > 999;        -- safety cap
    END LOOP;
    dbg('unique_workbook_name: resolved → "' || v_name || '"');
    RETURN v_name;
  END;
  ...
  PROCEDURE delete_export(p_export_uid IN VARCHAR2) IS
    ...
    v_cnt   PLS_INTEGER;
  BEGIN
    dbg('delete_export: UID="' || p_export_uid || '"');
    ...
    dbg('delete_export: removed "' || v_name || '"');
